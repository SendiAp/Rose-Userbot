...
from userbot import CMD_HANDLER, CMD_LIST, DEFAULT, DEVS, ROS2, ROS3, ROS4, ROS5, bot
...
def register(**args):
    ...
    def decorator(func):
        async def wrapper(check):
            ...
            try:
                ...
                if not disable_errors:
                    ...
                    text = "**✘ ROSE USERBOT ERROR REPORT ✘**\n\n"
                    link = "[Group Support](https://t.me/BottyCu)"
        ...
        if ROS2:
            if not disable_edited:
                ROS2.add_event_handler(wrapper, events.MessageEdited(**args))
            ROS2.add_event_handler(wrapper, events.NewMessage(**args))
        if ROS3:
            if not disable_edited:
                ROS3.add_event_handler(wrapper, events.MessageEdited(**args))
            ROS3.add_event_handler(wrapper, events.NewMessage(**args))
        if ROS4:
            if not disable_edited:
                ROS4.add_event_handler(wrapper, events.MessageEdited(**args))
            ROS4.add_event_handler(wrapper, events.NewMessage(**args))
        if ROS5:
            if not disable_edited:
                ROS5.add_event_handler(wrapper, events.MessageEdited(**args))
            ROS5.add_event_handler(wrapper, events.NewMessage(**args))
        return wrapper
